...
var Auth = function () {
  ...
  _createClass(Auth, [{
    ...
    value: function checkAuth(to, query, next) {
      ...
      if (this.options.allowThirdpartyLogin && query.thirdparty_connect_access_token && query.thirdparty_connect_refresh_token) {
          ...
          this.fetch().then(function () {
            ...
            if ((typeof auth === 'undefined' ? 'undefined' : _typeof(auth)) === 'object' && auth.constructor === Array) {
              if (!user.role && !auth.length || auth.indexOf(user.role) !== -1) {
                ...
                next(_this3.options.forbiddenRedirect);
              }
            } else if (auth === user.role || auth === true) {
              next();
            } else {
              next(_this3.options.authRedirect);
            }
          }, function () {
            // 获取用户信息失败，跳转到登录页面
            next(_this3.options.authRedirect);
