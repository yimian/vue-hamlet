...
var Auth = function () {
  function Auth(Vue) {
    ...
    Vue.router.beforeEach(function (to, from, next) {
      ...
      //当链接中带有第三方登录token时, 将token放入localStorage, 并重定向到去除了token的链接
      ...
      if (auth) {
        ...
        if (!_this2._store.state.auth.token) {
          ...
          next({ path: _this2.options.authRedirect, query: { redirectedFrom: to.redirectedFrom } });
        } else {
  ...
  _createClass(Auth, [{
    ...
    value: function login(_ref) {
      ...
      var _this = this;
      return this._http.post(url, { username: username, password: password }).then(function (res) {
        if (res.body.ok) {
    ...
    value: function logout() {
      ...
      var _this = this;
      return this._http.post(url).then(function () {
    ...
    value: function refresh() {
      ...
      var _this = this;
      return this._http.get(url, { params: { refresh_token: _this._store.state.auth.refresh_token } }).then(function (res) {
        if (res.body.ok) {
          _this._store.commit(types.SET_TOKEN, res.body.data.access_token);
          return res;
        }
